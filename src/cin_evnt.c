...
e_cinefxevent_node_t* firstevent;
e_cinefxevent_node_t* lastevent;
...
e_cinefxevent_node_t* GetNewEvent(void)
{
	...
	event = SafeMalloc(sizeof(e_cinefxevent_node_t));
...
void ShutdownEvents(void)
{
	...
	while (event != NULL)
	{
		e_cinefxevent_node_t* nextevent;
...
===============
*/
e_cinefxevent_t GetEventType(void)
{
	// Get Event Token
...
===============
*/
void ParseBack(e_cinefxevent_node_t* event)
{
	char name1[10];
	char name2[10];
	int duration;
		...
		(event->effecttype == background_scrolling))
	{
		...
		lpic = (lpic_t*)W_CacheLumpName(name1, PU_CACHE, Cvt_lpic_t, 1);
		width = lpic->width;
	}
	else if (event->effecttype != background_multi)
	{
		...
		patch = (patch_t*)W_CacheLumpName(name1, PU_CACHE, Cvt_lpic_t, 1);
		width = patch->width;
	}
	...
	if (event->effecttype == background_multi)
		event->effect = SpawnCinematicMultiBack(name1, name2, duration, startx,
												endx, yoffset);
	else
		event->effect =
			SpawnCinematicBack(name1, duration, width, startx, endx, yoffset);
}
...
===============
*/
void ParseSprite(e_cinefxevent_node_t* event)
{
	char name1[10];
	int duration;
	int numframes;
	...
	event->effect = SpawnCinematicSprite(name1, duration, numframes, framedelay,
...
void ParseFlic(e_cinefxevent_node_t* event)
{
	...
	event->effect = SpawnCinematicFlic(name1, loop, usefile);
}
...
===============
*/
void ParsePalette(e_cinefxevent_node_t* event)
{
	char name1[10];
	...
	event->effect = SpawnCinematicPalette(name1);
}
