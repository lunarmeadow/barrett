...

{
	...
	memset(&Messages[num], 0, sizeof(message_t));
...

{
	...
	messagetics = GetCachedTic() - LastMessageTime;
	LastMessageTime = GetCachedTic();
...

{
	...
	for (i = 0; i < numplayers; i++)
	{
		...
		{
			strncpy(str, "0 - ", 4);
			strncat(str, PLAYERSTATE[i].codename,
					strlen(PLAYERSTATE[i].codename));
			str[0] = '0' + p;
			p++;
			if (p > 9)
			{
