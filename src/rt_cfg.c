...
void CheckVendor(void)
{
	...
	if (IS_SHAREWARE)
	{
		VENDORDOC = DATADIR "VENDOR.DOC";
		VENDORLUMP = "VENDOR";
	}
	else
	{
		VENDORDOC = DATADIR "LICENSE.DOC";
		VENDORLUMP = "LICENSE";
	}
...

{
	...
	GetPathFromEnvironment(filename, ApogeePath, BattleName);
	file =
		open(filename, O_RDWR | O_TEXT | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);
...
void WriteConfig(void)
{
	...
	SafeWriteString(file, "\n;\n");
	SafeWriteString(file, "; 1 - Bullet weapons will automatically target "
						  "enemies. (ROTT default)\n");
	...
	SafeWriteString(file, "\n;\n");
	SafeWriteString(file, "; 1 - Missile weapons will be automatically aimed "
						  "at targets like bullet weapons.\n");
