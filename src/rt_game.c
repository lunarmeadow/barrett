...
HighScore Scores[MaxScores] = {
	{"Tom", 70000, 7, 1},	   {"Chuck", 60000, 6, 1},	 {"Mark", 50000, 5, 1},
	{"The Joes", 40000, 4, 1}, {"William", 30000, 3, 1}, {"Jim", 20000, 2, 1},
	{"Steve", 10000, 1, 1},
};
...

{
	...
	else
	{
		...
		for (i = 0; i < numplayers; i++)
		{
			...
			{
				negfragpic[j] =
					(pic_t*)W_CacheLumpName("teamnpic", PU_LEVEL, Cvt_pic_t, 1);
				fragpic[j] =
...
void DrawPlayScreen(bool bufferofsonly)
{
	...
	if (!BATTLEMODE)
	{
		...
		GameMemToScreen(men[character], MEN_X + topBarCenterOffsetX, MEN_Y,
						bufferofsonly);
		...
		DrawGameString(MEN_X + 3 + topBarCenterOffsetX, MEN_Y + 2,
					   Names[character], bufferofsonly);
	...
	if (locplayerstate->poweruptime)
	{
		...
		GameMemToScreen(eraseb, POWERUP1X + topBarCenterOffsetX, POWERUPY,
						bufferofsonly);

		DrawMPPic(POWERUP1X + topBarCenterOffsetX, POWERUPY + powerupheight,
	...
	if (locplayerstate->protectiontime)
	{
		...
		GameMemToScreen(eraseb, POWERUP2X + topBarCenterOffsetX, POWERUPY,
						bufferofsonly);
...
void DrawKills(bool bufferofsonly)
{
	...
	if (SHOW_TOP_STATUS_BAR())
	{
		...
		DrawPPic(LEADER_X, LEADER_Y, pic->width, pic->height, (byte*)&pic->data,
...
void DrawNumber(int x, int y, int width, int which, bool bufferofsonly)
{
	...
	while (c < length)
	{
		...
		case 3:
			...
			break;
		case 4:
			StatusDrawColoredPic(x, y, scorenums[str[c] - '0'], bufferofsonly,
								 playeruniformcolor);
			x += 8;
			break;
		case 5:
...

{
	...
	if (!BATTLEMODE)
	{
		DrawNumber(SCORE_X + topBarCenterOffsetX, SCORE_Y, 10, 1,
				   bufferofsonly);
	}
}
...
void DrawKeys(bool bufferofsonly)
{
	...
	{
		GameMemToScreen(keys[0], KeyX[0] + topBarCenterOffsetX, KEY_Y,
						bufferofsonly);
	}

	if (locplayerstate->keys & 2)
	{
		GameMemToScreen(keys[1], KeyX[1] + topBarCenterOffsetX, KEY_Y,
						bufferofsonly);
	}

	if (locplayerstate->keys & 4)
	{
		GameMemToScreen(keys[2], KeyX[2] + topBarCenterOffsetX, KEY_Y,
						bufferofsonly);
	}

	if (locplayerstate->keys & 8)
	{
		GameMemToScreen(keys[3], KeyX[3] + topBarCenterOffsetX, KEY_Y,
						bufferofsonly);
	}
}
...

{
	...
	{
		oldsec = sec;
		DrawTimeXY(GAMETIME_X + topBarCenterOffsetX, GAMETIME_Y, sec,
				   bufferofsonly);
	}
}
...

{
	...
	if (!time)
	{
		...
		GM_MemToScreen((byte*)&eraseb->data, eraseb->width, eraseb->height,
					   shapenum + topBarCenterOffsetX, POWERUPY);
		...
		if (time < (poweruptime - poweradjust))
		{
			...
			else
			{
				GM_MemToScreen((byte*)&eraseb->data, eraseb->width,
							   eraseb->height, POWERUP1X + topBarCenterOffsetX,
							   POWERUPY);
			...
			GM_MemToScreen((byte*)&eraseb->data, eraseb->width, eraseb->height,
						   POWERUP1X + topBarCenterOffsetX, POWERUPY);

			DrawMPPic(POWERUP1X + topBarCenterOffsetX, POWERUPY + powerupheight,
					  shape->width, shape->height - powerupheight,
					  powerupheight, (byte*)&shape->data, false);
		}
	}
	else
	{
		if (time < (protectiontime - poweradjust))
		{
			...
			GM_MemToScreen((byte*)&eraseb->data, eraseb->width, eraseb->height,
						   POWERUP2X + topBarCenterOffsetX, POWERUPY);

			DrawMPPic(POWERUP2X + topBarCenterOffsetX,
					  POWERUPY + protectionheight, shape->width,
...
void ScreenShake(void)
{
	...
	if (SHAKETICS != 0xFFFF)
	{
		...
		// fix for play screen accidentally being drawn during transmitter
		// explosion cinematic
...
void Died(void)
{
	...
	if (locplayerstate->lives > -1)
	{
		...
		if (pstate->falling == true)
		{
			...
			// (VBLCOUNTER*(1+slowrate))); RotateScreen (0, 0, (FINEANGLES),
			// (FINEANGLES>>6), (VBLCOUNTER*(1+slowrate)), 1, true);
			RotateScreenScaleFloat(0, 0, 1.0, 64.0,
								   (VBLCOUNTER * (1 + slowrate)), true, false);
		...
		{
			// RotateBuffer (0, 0, (FINEANGLES>>6), (FINEANGLES),
			// (VBLCOUNTER*(2+slowrate))); RotateScreen (0, 0, (FINEANGLES),
			// (FINEANGLES*64), (VBLCOUNTER*(2+slowrate)), 2, true);
			RotateScreenScaleFloat(0, 0, 1.0, 0.01875,
								   (VBLCOUNTER * (2 + slowrate)), true, false);
		}
		// zooms in on screen
		else if (rng < 128)
		{
		...
		else if (rng < 192)
		{
			...
			RotateScreenScaleFloat(0, (360.0 * 3), 1.0, 0.01875,
								   (VBLCOUNTER * (3 + slowrate)), true, false);
		}
		// fade to red
		else
		{
	...
	else
	{
		...
		if (rng < 64)
		{
			...
			// RotateScreen(0,(FINEANGLES>>1),(FINEANGLES),(FINEANGLES*64),(VBLCOUNTER*(3+slowrate)),
			// 2, true);
			RotateScreenScaleFloat(0, 360, 1.0, 0.01875,
								   (VBLCOUNTER * (3 + slowrate)), true, false);
		}
		else if (rng < 128)
		{
		...
		else if (rng < 192)
		{
			...
			// RotateScreen(0,(FINEANGLES*2),(FINEANGLES),(FINEANGLES*64),(VBLCOUNTER*(3+slowrate)),
			// 2, true);
			RotateScreenScaleFloat(0, 360 * 2, 1.0, 0.01875,
								   (VBLCOUNTER * (3 + slowrate)), true, false);
		}
		else
		{
