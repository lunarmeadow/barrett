...
static char* TimeLimitOptionDescriptions[8] = {
	"One Minute", "2 Minutes",	"5 Minutes",  "10 Minutes",
...
CP_itemtype LSMenu[] = {{CP_Active, "", 'a', NULL}, {CP_Active, "", 'b', NULL},
...
CP_itemtype ControlSelectMenu[] = {
	{2, "\0", 'N', NULL}, {1, "\0", 'L', NULL}, {1, "\0", 'R', NULL},
...
CP_MenuNames ControlMMenuNames[] = {
	"VISUAL OPTIONS",	"CONTROLS",		  "USER OPTIONS",
	"EXT USER OPTIONS", // bna added
	"EXT GAME OPTIONS", // added by LT
	"MUSIC VOLUME",		"SOUND FX VOLUME"

};
CP_iteminfo ControlMItems = {32,		  48 - 8, 7, 0, 32, ControlMMenuNames,
...
CP_MenuNames ExtGameOptionsNames[] = {"LOW MEMORY MODE", "WEAPON RECOLOURS", "DOOM BOBBING", "NO LOW HP SOUND", "HALF MONK HEALTH", "LUDICROUS SKYBOX", "LUDICROUS AUDIO"}; // erysdren added
...
(??)	20, MENU_Y, 7, 0, 43, ExtGameOptionsNames, mn_largefont}; // LT added
...
CP_MenuNames ModeMenuNames[] = {"NORMAL",	 "SCORE MORE", "COLLECTOR",
								"SCAVENGER", "HUNTER",	   "TAG",
								"ELUDER",	 "DELUDER",	   "CAPTURE THE TRIAD"};
...
CP_iteminfo AmmoPerWeaponItems = {
	32, 26, 3, 0, 45, AmmoPerWeaponMenuNames, mn_largefont};
...
CP_MenuNames SpawnMenuNames[] = {
	"SPAWN DANGERS", "SPAWN HEALTH",	   "SPAWN WEAPONS",	 "SPAWN MINES",
...
CP_iteminfo MultiPageCustomItems = {
	18, 17, 0, 0, 12, MultiPageCustomNames, mn_smallfont};
CP_itemtype MultiPageCustomMenu[] = {
	{1, "", 'N', NULL}, {1, "", 'P', NULL}, {1, "", 'a', NULL},
	{1, "", 'a', NULL}, {1, "", 'a', NULL}, {1, "", 'a', NULL},
	{1, "", 'a', NULL}, {1, "", 'a', NULL}, {1, "", 'a', NULL},
	{1, "", 'a', NULL}, {1, "", 'a', NULL}, {1, "", 'a', NULL},
	{1, "", 'a', NULL}, {1, "", 'a', NULL},
};
...
void DrawVisualsMenu(void)
{
	...
	DrawMenuBufItem(
		VisualOptionsItems.x,
		((VisualOptionsItems.curpos * 14) + (VisualOptionsItems.y - 2)),
		W_GetNumForName(LargeCursor) + CursorFrame[CursorNum]);
	DisplayInfo(0);
	FlipMenuBuf();
}
...
void DoAdjustHudScale(void)
{
	...
	ConditionalLimitSliderMenu(&hudRescaleFactor, 12, 1, maxRescale, 44, 81, 194, 1, "block2", NULL, "Adjust HUD Scaling", "Factor: ", "x");
	...
	if (ScreenResolutions == NULL)
	{
		...
		for (i = 0; i < nbrResolutions; i++)
		{
			...
			if (AvailableResolutions[i].extraMenuText)
				snprintf(ScreenResolutions[i], 64, "%dx%d (%s)",
						 AvailableResolutions[i].width,
						 AvailableResolutions[i].height,
						 AvailableResolutions[i].extraMenuText);
			else
				snprintf(ScreenResolutions[i], 64, "%dx%d",
						 AvailableResolutions[i].width,
						 AvailableResolutions[i].height);
		...
		ScreenResolutionItems =
			(CP_iteminfo){NORMALKEY_X,		 17,		 nbrResolutions, 0, 16,
						  ScreenResolutions, mn_tinyfont};
	}

	// Find the current resolution and select the resolution
	position =
		0; // Default select first if current resolution isn't in the list
...
void CP_ExtOptionsMenu(void)
{
	...
	do
	{
		which = HandleMenu(&ExtOptionsItems, &ExtOptionsMenu[0],
						   DrawExtOptionDescription);
	...
	for (i = 0; i < ExtOptionsItems.amount; i++)
		if (ExtOptionsMenu[i].active != CP_Active3)
		{
			...
			case 1:
				...
				break;
			case 2:
				if (allowMovementWithMouseYAxis == 1)
					on = 1;
				break;
			case 3:
				if (iG_aimCross == 1)
...
void DrawExtGameOptionsButtons(void)
{
	...
	for (i = 0; i < ExtGameOptionsItems.amount; i++)
		if (ExtGameMenu[i].active != CP_Active3)
		{
			...
			if (on)
				DrawMenuBufItem(20 + 22, ExtGameOptionsItems.y + i * 14 - 1,
								button_on);
			else
				DrawMenuBufItem(20 + 22, ExtGameOptionsItems.y + i * 14 - 1,
								button_off);
...
void CP_ExtGameOptionsMenu(void)
{
	...
	do
	{
		...
		which = HandleMenu(&ExtGameOptionsItems, &ExtGameMenu[0],
						   DrawExtGameOptionDescription);
