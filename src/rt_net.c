...
void ResendPacket(void* pkt, int dest)
{
	...
	ComError("RESEND request received at %d\n packet time=%d dest=%d "
			 "numpackets=%d\n",
			 GetCachedTic(), time, dest, request->numpackets);
...
void ProcessSyncCheckPacket(void* pkt, int src)
{
	...
	if (sync->x != PlayerSync[src].x)
	{
		Error("Player %d is unsynced local x=%d remote x=%d\n"
			  "Unsynced Player x=%x y=%x a=%d z=%d name=%s\n",
			  src, PlayerSync[src].x, sync->x, PlayerSync[src].x,
			  PlayerSync[src].y, PlayerSync[src].angle, PlayerSync[src].z,
	...
	if (sync->y != PlayerSync[src].y)
	{
		Error("Player %d is unsynced local y=%d remote y=%d\n"
			  "Unsynced Player x=%x y=%x a=%d z=%d name=%s\n",
			  src, PlayerSync[src].y, sync->y, PlayerSync[src].x,
			  PlayerSync[src].y, PlayerSync[src].angle, PlayerSync[src].z,
			  PLAYERSTATE[src].codename);
	}
	if (sync->z != PlayerSync[src].z)
	{
		Error("Player %d is unsynced local z=%d remote z=%d\n"
			  "Unsynced Player x=%x y=%x a=%d z=%d name=%s\n",
			  src, PlayerSync[src].z, sync->z, PlayerSync[src].x,
			  PlayerSync[src].y, PlayerSync[src].angle, PlayerSync[src].z,
			  PLAYERSTATE[src].codename);
	}
	if (sync->angle != PlayerSync[src].angle)
	{
		Error("Player %d is unsynced local angle=%d remote angle=%d\n"
			  "Unsynced Player x=%x y=%x a=%d z=%d name=%s\n",
			  src, PlayerSync[src].angle, sync->angle, PlayerSync[src].x,
			  PlayerSync[src].y, PlayerSync[src].angle, PlayerSync[src].z,
			  PLAYERSTATE[src].codename);
	}
}
...
void SendSyncCheckPacket(void)
{
	...
	((COM_CheckSyncType*)NextLocalCommand())->angle =
		PlayerSync[consoleplayer].angle;
	((COM_CheckSyncType*)NextLocalCommand())->randomindex =
		PlayerSync[0].randomindex;
...
void SyncToServer(void)
{
	...
	//      {
	//      diff =
	//      (GetCachedTic()-controldivisor-LastCommandTime[0])/controldivisor;
	//      SoftError("diff=%ld\n",diff);
	//      if (abs(diff)>1)
	//         ISR_SetTime(GetCachedTic()-diff);
	//      }
	//   else
	//      {
	diff = (GetCachedTic() - controldivisor - LastCommandTime[server]) /
		   controldivisor;
	if (abs(diff) > 0)
		ISR_SetTime(GetCachedTic() - diff);
	//      }
...
void ProcessServer(void)
{
	...
	if (GetCachedTic() < serverupdatetime)
	...
	while (time >= serverupdatetime)
	{
		...
		savetime = GetCachedTic() + NETWORKTIMEOUT;
		done = false;
		while (done == false)
		{
			if (standalone == true)
...
void ControlPlayerObj(objtype* ob)
{
	...
	if (oldpolltime == nextupdatetime)
	{
		...
		if (networkgame == true)
			...
			savetime = GetCachedTic() + MODEMTIMEOUT;
...
void SetupGamePlayer(void)
{
	...
	while (done == false)
	{
		...
		if (GetCachedTic() >= savetime)
		{
			savetime = GetCachedTic() + SETUPTIME;
			if (gameready == false)
				SendPlayerDescription();
			else
				SendGameAck();
	...
	savetime = GetCachedTic() + (VBLCOUNTER / 2);

	while (GetCachedTic() < savetime)
	{
		SetupCheckForPacket();
	}
}
