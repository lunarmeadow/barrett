...
statinfo stats[NUMSTATS] = {
	...
	{0, SPR5_LAMPOFF, stat_lamp, FL_LIGHT | FL_BLOCK | FL_SHOOTABLE, 0, 0, 2, 0,
	 0},
	...
	{0, SPR73_GKEY1, stat_pedironkey,
	 FL_COLORED | FL_BONUS | FL_CHANGES | FL_BLOCK | FL_ACTIVE, 2, 16, pc_olive,
	 0, 0},
	...
	{0, TORCH1, stat_altbrazier1,
	 FL_HEAT | FL_BLOCK | FL_ACTIVE | FL_LIGHT | FL_SHOOTABLE, 2, 15, 2, 0, 0},
	...
	{0, BAT1, stat_bat, FL_BONUS | FL_ACTIVE | FL_RESPAWN | FL_WEAPON, 1, 16, 0,
	 0, 10},
	{0, KNIFE_STATUE1, stat_knifestatue, FL_BONUS | FL_CHANGES | FL_BLOCK, 0, 0,
	 0, 0, 0},
	{0, SPR_TWOPIST, stat_twopistol, FL_BONUS | FL_RESPAWN | FL_WEAPON, 0, 0, 0,
	 0, 5},
	...
	{0, SPR_BAZOOKA, stat_bazooka, FL_BONUS | FL_RESPAWN | FL_WEAPON, 0, 0, 0,
	 0, 10},
	{0, SPR_FIREBOMB, stat_firebomb, FL_BONUS | FL_RESPAWN | FL_WEAPON, 0, 0, 0,
	 0, 5},
	...
	{0, SPR_DRUNK, stat_drunkmissile, FL_BONUS | FL_RESPAWN | FL_WEAPON, 0, 0,
	 0, 0, 7},
	{0, SPR_FIREWALL, stat_firewall, FL_BONUS | FL_RESPAWN | FL_WEAPON, 0, 0, 0,
	 0, 5},
	...
	{24, SPR32_EXPLOS, stat_tntcrate, FL_SHOOTABLE | FL_BLOCK | FL_WOODEN, 0, 0,
	 3, 10, 0},
	{12, SPR33_CBARREL, stat_bonusbarrel, FL_METALLIC | FL_SHOOTABLE | FL_BLOCK,
	 0, 0, 3, 10, 0},
	{0, TORCH1, stat_torch,
	 FL_BLOCK | FL_LIGHT | FL_ACTIVE | FL_HEAT | FL_SHOOTABLE, 2, 15, 2, 0, 0},
	{30, FFLAME1, stat_floorfire,
	 FL_HEAT | FL_BLOCK | FL_LIGHT | FL_ACTIVE | FL_SHOOTABLE, 2, 7, 30, 0, 0},
	...
	{0, SCOTHEAD1, stat_scotthead, FL_BONUS | FL_ACTIVE | FL_FULLLIGHT, 4, 7, 0,
	 0, 0},
	...
	{0, BULLETPROOFPOWERUP, stat_bulletproof, FL_BONUS | FL_RESPAWN, 0, 0, 0, 0,
	 0},
	{0, ASBESTOSPOWERUP, stat_asbesto, FL_BONUS | FL_RESPAWN, 0, 0, 0, 0, 0},
	{0, RANDOMPOWERUP1, stat_random, FL_BONUS | FL_ACTIVE | FL_RESPAWN, 2, 8, 0,
	 0, 0},
...
#endif
	{0, SPR_MINE1, stat_mine, FL_BONUS | FL_SHOOTABLE | FL_RESPAWN, 0, 0, 10, 0,
	 0},
...
void PreCacheStaticFrames(statobj_t* temp)
{
	...
	case stat_godmode:
		...
		PreCacheGroup(W_GetNumForName("GODFIRE1"), W_GetNumForName("GODFIRE4"),
					  cache_patch_t);
...
void Set_NewZ_to_MapValue(fixed_t* newz, int zoffset, const char* errorstr,
						  int tilex, int tiley)
{
	int zf, z;
	...
	if (BATTLEMODE)
	{
		...
		case stat_healingbasin:
			if ((gamestate.Product != ROTT_SHAREWARE) &&
				...
				(!IsPlatform(tilex, tiley) && ((zoffset & 0xff00) != 0xb000)) &&
				(zoffset == -1))
			{
				mtype = stat_mine;
			}
		...
		else
		{
			...
			else if (temp->itemnumber == stat_standardpole)
			{
				if (MAPSPOT(temp->tilex, temp->tiley, 2))
					temp->count =
						2 * (MAPSPOT(temp->tilex, temp->tiley, 2) - 14);
				else
					temp->count = 0;
			}
