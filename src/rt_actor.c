...
static int starthitpoints[4][NUMENEMIES + 2] =
	...
	{{0, 0, 30, 35, 50, 40, 45, 425, 200, 200, 100, 1500, 2500, 3000, 3000, -1,
	  200, 2},
	 {0, 0, 40, 50, 55, 50, 50, 475, 250, 250, 125, 2300, 3400, 4500, 3600, -1,
	  250, 2},
	 {0, 0, 50, 65, 60, 60, 60, 525, 275, 300, 150, 2400, 3600, 5000, 4500, -1,
	  300, 2},
	 {0, 0, 60, 80, 70, 70, 75, 525, 300, 350, 175, 2800, 3800, 5900, 4800, -1,
	  350, 2}};
...
static const byte dirorder16[16][2] = {
	{15, 1}, {0, 2},  {1, 3},  {2, 4},	 {3, 5},   {4, 6},	 {5, 7},   {6, 8},
...
static statetype* UPDATE_STATES[NUMSTATES][NUMENEMIES] =
	...
	{{&s_lowgrdstand, &s_highgrdstand, &s_opstand, &s_strikestand,
	  ...
	  &s_robogrdstand, &s_darianstand, &s_heinrichstand, NULL, &s_darkmonkstand,
	  NULL, &s_gunstand, &s_wallstand},
	 ...
	 {&s_lowgrdcollide, &s_highgrdcollide, &s_opcollide, &s_strikecollide,
	  ...
	  &s_robogrdcollide, &s_dariancollide, NULL, NULL, NULL, NULL, NULL,
	  &s_wallcollide},
...
static statetype* UPDATE_STATES[NUMSTATES][NUMENEMIES] =
	...
	{{&s_lowgrdstand, &s_highgrdstand, NULL, &s_strikestand, &s_blitzstand,
	 ...
	 {&s_lowgrdchase1, &s_highgrdchase1, NULL, &s_strikechase1, &s_blitzchase1,
	  &s_enforcerchase1, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
	  NULL},
	 ...
	 {&s_lowgrdshoot1, &s_highgrdshoot1, NULL, &s_strikeshoot1, &s_blitzshoot1,
	  &s_enforcershoot1, NULL, NULL, &s_robogrdshoot1, NULL, NULL, NULL, NULL,
	  NULL, NULL, NULL},

	 {&s_lowgrddie1, &s_highgrddie1, NULL, &s_strikedie1, &s_blitzdie1,
	  &s_enforcerdie1, NULL, NULL, &s_robogrddie1, NULL, NULL, NULL, NULL, NULL,
	  NULL, NULL},
...
{
	if (DoPanicMapping() &&
		((newstate == &s_explosion1) || (newstate == &s_grexplosion1) ||
...
void ConsiderAlternateActor(objtype* ob, classtype which)
{
		...
		(which == dfiremonkobj))
	{
		if (GameRandomNumber("SpawnStand", which) < 128)
		{
			...
			case lowguardobj:
				ob->shapeoffset =
					W_GetNumForName("MARSHOO1") - W_GetNumForName("LWGSHOO1");
				break;
			case highguardobj:
				ob->shapeoffset =
					W_GetNumForName("HIGSHOO1") - W_GetNumForName("HG2SHOO1");
				break;
			case overpatrolobj:
				ob->shapeoffset =
					W_GetNumForName("PATSHOO1") - W_GetNumForName("OBPSHOO1");
				break;
			case strikeguardobj:
				ob->shapeoffset =
					W_GetNumForName("XYGSHOO1") - W_GetNumForName("ANGSHOO1");
				break;
			/*case blitzguardobj:
...
void SpawnStand(classtype which, int tilex, int tiley, int dir, int ambush)
{
	...
	case b_heinrichobj:
	case b_darkmonkobj:
		Error("\n%s actor at %d,%d not allowed in shareware !", debugstr[which],
			  tilex, tiley);
		break;
	default:;
	}
...
void SpawnParticles(objtype* ob, int which, int numparticles)
{
	...
	for (i = 0; i < numparticles; i++)
	{
		...
		// randadj = RandomSign() * (GameRandomNumber("rand gib adjust",0) >>
		// 4);
	...
	for (y = tileylow; y <= tileyhigh; y++)
		for (x = tilexlow; x <= tilexhigh; x++)
		{
			...
			if (tempwall && M_ISWALL(tempwall) && (tempwall->which != MWALL))
			{
				...
				if ((ocl == p_bazookaobj || ocl == p_firebombobj ||
					 ocl == p_heatseekobj || ocl == p_drunkmissileobj ||
					 ocl == p_firewallobj || ocl == p_splitmissileobj ||
					 ocl == p_kesobj) &&
					ricochetingRocketsEnabled)
				{
					if (WallCheck(ob->x - ob->momentumx, tryy))
					{
						ob->momentumx = -ob->momentumx;
...
#define PWALLTOL (0xc000)
		...
		else
		{
			...
			Collision(temp, (objtype*)pwall, pwall->momentumx - temp->momentumx,
	...
	if (tempstat->hitpoints <= 0)
	{
		...
		if (tempstat->flags & FL_LIGHT)
		{
			...
			else
			{
				...
				case stat_torch:
					SpawnSlowParticles(gt_sparks, 5, tempstat->x, tempstat->y,
									   tempstat->z - 32);
					break;
				case stat_floorfire:
					SpawnSlowParticles(gt_sparks, 5, tempstat->x, tempstat->y,
...
#if (SHAREWARE == 0)
			case stat_mine:
				SpawnNewObj(tempstat->tilex, tempstat->tiley, &s_grexplosion1,
							inertobj);
				MakeActive(new);
				new->flags |= FL_ABP;
				new->whatever = tempstat;
...
//if (!(ob->flags & FL_SHOOTABLE))
	...
	{   if (ob->obclass == strikeguardobj)
		...
		target = (objtype*)(ob->target);
		if (!target)
			Error("an instance of %s called shoot without a
target\n",debugstr[ob->obclass]);
		...
		accuracy=FixedMulShift(num,accuracy,8); // scale accuracy based off
randomness
