...

{
	...
	case MV_DMAFailure:
		ErrorString = "Playback failed, possibly due to an invalid or "
					  "conflicting DMA channel.";
		break;
...

{
	...
	if ((voice->length == 0) && (voice->GetSound != NULL) &&
...

{
	...
	else
	{
		...
		while (length > 0)
		{
			...
			// if we go through the loop again, it means that we've wrapped
			// around the buffer
...

{
	...
	while (!done)
	{
		...
		case 0:
			...
			{
				voice->BlockLength =
					(ptr - 4) - (unsigned char*)voice->LoopStart;
				voice->sound = voice->LoopStart;
				voice->position = 0;
				voice->length = min(voice->BlockLength, 0x8000);
				voice->BlockLength -= voice->length;
	...
	if (voice->Playing)
	{
		...
		voice->RateScale =
			(voice->SamplingRate * voice->PitchScale) / MV_MixRate;

		// Multiply by MixBufferSize - 1
		voice->FixedPointBufferSize =
			(voice->RateScale * MixBufferSize) - voice->RateScale;
...

{
	...
	// Start playback
	status = DSL_BeginBufferedPlayback(MV_MixBuffer[0], TotalBufferSize,
									   MV_NumberOfBuffers, MV_RequestedMixRate,
...
int MV_PlayRaw(char* ptr, unsigned long length, unsigned rate, int pitchoffset,
...

{
	...
	status = MV_PlayLoopedRaw(ptr, length, NULL, NULL, rate, pitchoffset, vol,
...
int MV_PlayLoopedRaw(char* ptr, unsigned long length, char* loopstart,
...
int MV_PlayWAV(char* ptr, int pitchoffset, int vol, int left, int right,
...

{
	...
	status =
		MV_PlayWAV(ptr, pitchoffset, mid, left, right, priority, callbackval);
...
int MV_PlayLoopedWAV(char* ptr, long loopstart, long loopend, int pitchoffset,
					 int vol, int left, int right, int priority,
					 unsigned long callbackval)
...

{
	...
	status =
		MV_PlayVOC(ptr, pitchoffset, mid, left, right, priority, callbackval);
...
int MV_PlayLoopedVOC(char* ptr, long loopstart, long loopend, int pitchoffset,
					 int vol, int left, int right, int priority,
					 unsigned long callbackval)
