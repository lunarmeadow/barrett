...

{
	...
	case FX_ASSVersion:
		ErrorString =
			"Apogee Sound System Version " ASS_VERSION_STRING "  "
...

{
	...
	handle =
		MV_PlayVOC(ptr, pitchoffset, vol, left, right, priority, callbackval);
	if (handle < MV_Ok)
	{
		FX_SetErrorCode(FX_MultiVocError);
		handle = FX_Warning;
...
int FX_PlayLoopedVOC(char* ptr, long loopstart, long loopend, int pitchoffset,
...

{
	...
	handle = MV_PlayLoopedVOC(ptr, loopstart, loopend, pitchoffset, vol, left,
...

{
	...
	handle =
		MV_PlayWAV(ptr, pitchoffset, vol, left, right, priority, callbackval);
	if (handle < MV_Ok)
	{
		FX_SetErrorCode(FX_MultiVocError);
		handle = FX_Warning;
...
int FX_PlayLoopedWAV(char* ptr, long loopstart, long loopend, int pitchoffset,
					 int vol, int left, int right, int priority,
					 unsigned long callbackval)

{
	...
	handle = MV_PlayLoopedWAV(ptr, loopstart, loopend, pitchoffset, vol, left,
							  right, priority, callbackval);
	if (handle < MV_Ok)
	{
		FX_SetErrorCode(FX_MultiVocError);
...

{
	...
	handle =
		MV_PlayVOC3D(ptr, pitchoffset, angle, distance, priority, callbackval);
	if (handle < MV_Ok)
	{
		FX_SetErrorCode(FX_MultiVocError);
		handle = FX_Warning;
...

{
	...
	handle =
		MV_PlayWAV3D(ptr, pitchoffset, angle, distance, priority, callbackval);
	if (handle < MV_Ok)
	{
		FX_SetErrorCode(FX_MultiVocError);
		handle = FX_Warning;
...

{
	...
	handle = MV_PlayRaw(ptr, length, rate, pitchoffset, vol, left, right,
...

{
	...
	handle =
		MV_PlayLoopedRaw(ptr, length, loopstart, loopend, rate, pitchoffset,
...
int FX_StartDemandFeedPlayback(void (*function)(char** ptr,
...

{
	...
	handle = MV_StartDemandFeedPlayback(function, rate, pitchoffset, vol, left,
										right, priority, callbackval);
	if (handle < MV_Ok)
	{
		FX_SetErrorCode(FX_MultiVocError);
