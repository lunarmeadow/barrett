...
int main(int argc, char* argv[])
{
	...
	{
		/* OS X will give us a path in the form '/Applications/Rise of the
		   Triad.app/Contents/MacOS/Rise of the Triad'. Our data is in
		   Contents/Resources. */
		char* path;
...
void DrawRottTitle(void)
{
	...
	if (CheckParm("QUIET") == 0)
	{
		...
		strncat(title, itoa(ROTTMAJORVERSION, &buf[0], 10),
				CountDigits(ROTTMAJORVERSION));
		strncat(title, ".", 1);
#if (SHAREWARE == 1) || (DOPEFISH == 0)
		strncat(title, itoa(ROTTMINORVERSION, &buf[0], 10),
				CountDigits(ROTTMINORVERSION));
#else
		strcat(title, "DFISH");
#endif
		strncat(title, "\n", 1);
...
void CheckCommandLineParameters(void)
{
		...
		((_argc > 1) && (_argv[1][0] == '?')))
	{
		...
		printf("         IJKL             - Look Up/Down\n");
		printf("         U/M              - Aim Up/Down\n");
...
void SetupWads(void)
{
	...
	int i, arg, argnum = 0;
	char* tempstr = NULL;
	char* PStrings[] = {"AIM",		  "FULLSCREEN", "WINDOW",
						"BORDERLESS", "RESOLUTION", NULL};
	...
	if (arg != 0)
	{
		...
		{ // are there a filename included
			...
			// strncpy (tempstr,_argv[arg+1], strlen(_argv[arg+1]));//copy it to
			// tempstr
			if (strlen(tempstr) < MAX_PATH)
			{
				...
				{ // try opnong file
					char cannotOpenStr[] =
						" not could be opened, skipping RTL file \n";
...
NoRTL:
	...
	if (arg != 0)
	{
		...
		{ // are there a filename included
			...
			tempstr = realloc(tempstr, templen);
			snprintf(tempstr, templen, "%s", _argv[arg + 1]);
			// strncpy (tempstr,_argv[arg+1], sizeof(&_argv[arg+1]));//copy it
			// to tempstr
			if (strlen(tempstr) < MAX_PATH)
			{
				if (access(tempstr, 0) != 0)
				...
				{ // try opening file
					char cannotOpenRTC[] =
						" could not be opened, skipping RTC file ";
...
skip_file_loading:
	...
	if (RemoteSounds.avail == true)
	{
		...
		tempstr = realloc(tempstr, strlen(RemoteSounds.path) +
	...
	while (1)
	{
		...
		case ex_bossdied:
			...
			if (IS_NOT_SHAREWARE && (playstate == ex_bossdied) && (gamestate.mapon != 30))
			{
				...
				LBM =
					(lbm_t*)W_CacheLumpName("deadboss", PU_CACHE, Cvt_lbm_t, 1);
