	...
	*/
	...
	if (loadedgame == false)
	{
		...
		for (i = 1; i < cacheindex; i++)
		{
			...
			{ // SetTextMode (  );
				// Note: This isn't drawing 4 LED thingys... its just drawing
				// one big one (made up of 4 of the LED thingys)

				DrawNormalSprite((int)(newPrecacheBarX + newPrecacheBar1LedX +
									   (Gs * (lastmem << 2))),
				...
				if (iGLOBAL_SCREENWIDTH > 320)
				{
					DrawNormalSprite(
						(int)(newPrecacheBarX + newPrecacheBar1LedX +
							  (Gs * (lastmem << 2))),
						(int)(newPrecacheBarY + newPrecacheBar1LedY + 3),
						W_GetNumForName("led1")); // led1 progressbar
				}
			...
			{
				DrawNormalSprite((int)(newPrecacheBarX + newPrecacheBar2LedX +
									   (Gs * (lastcache << 2))),
				...
				if (iGLOBAL_SCREENWIDTH > 320)
				{
					DrawNormalSprite(
						(int)(newPrecacheBarX + newPrecacheBar2LedX +
							  (Gs * (lastcache << 2))),
						(int)(newPrecacheBarY + newPrecacheBar2LedY + 3),
						W_GetNumForName("led2")); // led2 progressbar
					DrawNormalSprite(
						(int)(newPrecacheBarX + newPrecacheBar2LedX +
							  (Gs * (lastcache << 2)) + 3),
...

{
	...
	if (strcmp(RTLSignature, NORMAL_SIGNATURE_EX) == 0 ||
		strcmp(RTLSignature, COMMBAT_SIGNATURE_EX) == 0)
	{
				...
				// check to see we are not on a wall
				{
					...
					case 0:
						SpawnMaskedWall(i, j, mw_platform1,
										MW_BOTTOMPASSABLE | MW_MIDDLEPASSABLE);
						break;
					case 1:
					...
					case 4:
						SpawnMaskedWall(i, j, mw_platform5,
										MW_BOTTOMPASSABLE | MW_ABOVEPASSABLE);
						break;
					case 5:
					...
					default:
						Error(
							"Illegal Maskedwall platform value at x=%d y=%d\n",
							i, j);
						break;
					}
				}
		...
		for (i = 0; i < mapwidth; i++)
		{
			...
			case 80: // OldPushWall
				if (tilemap[i][j])
				{
					...
					tilemap[i][j] = pwallnum;
					if (MAPSPOT(i, j, 2))
						Error("You cannot link a pushwall which has no "
			...
			case 259:
				if (tilemap[i][j])
				{
					...
					Error("You have to place a turbomovewall icon on a wall at "
						  "x=%d y=%d",
						  i, j);
				break;
			...
			case 354:
				if (tilemap[i][j])
				{
					...
					Error("You have to place a movewall icon on a wall at x=%d "
						  "y=%d",
						  i, j);
				break;
			}
		...
		for (i = 0; i < mapwidth; i++)
		{
			...
			case 79:
				if (ActorIsPushWall(i, j))
				{
					if (MAPSPOT(i, j, 2))
					{
						...
						{
							Link_To_Touchplate(touchx, touchy, ActivatePushWall,
											   NULL, GetPushWallNumber(i, j),
											   0);
						}
						else
							Error("tried to link a pushwall at x=%d y=%d to a "
			...
			case 80:
				if (ActorIsPushWall(i, j))
				{
					if (MAPSPOT(i, j, 2))
					{
						Error("You shouldn't be linking a nondirectional-push "
							  "wall at x=%d y=%d\n",
							  i, j);
					}
				}
			...
			case 259:
				if (ActorIsPushWall(i, j))
				{
					if (MAPSPOT(i, j, 2))
					{
						...
						if (touchindices[touchx][touchy])
						{
							Link_To_Touchplate(touchx, touchy, ActivateMoveWall,
											   NULL, GetPushWallNumber(i, j),
											   0);
						}
						else
							Error("tried to link a turbomovewall at x=%d y=%d "
				...
				if (ActorIsPushWall(i, j))
				{
					if (MAPSPOT(i, j, 2))
					{
						...
						if (touchindices[touchx][touchy])
						{
							...
							Error("tried to link a movewall at x=%d y=%d to a "
								  "non-existent touchplate\n",
								  i, j);
					}
				}
...
{
	DoLowMemoryConversionBackgroundPlane();
	if ((modemgame == false) && (demorecord == false) &&
		(demoplayback == false))
		DoLowMemoryConversionForegroundPlane();
	DoLowMemoryConversionIconPlane();
}
	...
	for (j = 0; j < mapheight; j++)
	{
		...
		for (i = starti; i < mapwidth; i++)
		{
			...
			case 48:
				...
				if ((locplayerstate->player == 1) ||
					...
					PreCacheGroup(W_GetNumForName("RFPIST1"),
								  W_GetNumForName("LFPIST3"), cache_patch_t);

				else if (locplayerstate->player == 2)
					PreCacheGroup(W_GetNumForName("RBMPIST1"),
								  W_GetNumForName("LBMPIST3"), cache_patch_t);

				else
					PreCacheGroup(W_GetNumForName("RMPIST1"),
								  W_GetNumForName("LMPIST3"), cache_patch_t);
			...
			case 56:
				...
				PreCacheGroup(W_GetNumForName("KES1"), W_GetNumForName("KES6"),
							  cache_patch_t);
